...
date: 2021-01-16
description: MAML optimizes for rapid adaptation so a model can learn new tasks with only a few gradient steps, across supervised learning and RL.
...
*MSE :*  $$\mathcal{L}_{\tau_i} (f_\phi) = \sum_{x^{(j)},y^{(j)}\sim\tau_i} \lVert f_\phi(x^{(j)}) - y^{(j)}\rVert_2^2$$
