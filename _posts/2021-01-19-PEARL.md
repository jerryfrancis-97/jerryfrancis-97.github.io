...
date: 2021-01-19
description: PEARL learns task belief from context via a probabilistic encoder, making off-policy meta-RL both sample-efficient and easier to adapt.
...
Suppose that we have a prior experience in the form of history of past transitions, which we can call *'context'* , **$c$**. Here, $c^T_n = (s_n, a_n, r_n, s^{\prime}_n)$ be one transition in the task T so that $c^T_{1:N}$ comprises the experience collected so far. To encode the context into a latent form $Z$ the concept of variational inference is used, by training an inference network $q_{\phi}(z\mid c)$, parameterized by $\phi$, that estimates the posterior $p(z\mid c)$.
...
$$E_T \left[E_{z \sim q_{\phi}(z\mid c^T)} \left[R(T, z) + \beta D_{KL}\left(q_{\phi}(z\mid c^T) \,\Vert\, p(z)\right)\right]\right]$$
...
**Designing $q_{\phi}(z\mid c)$**

As the transitions follow *Markov* property, the ordering of $(s,a,s',r)$ tuples is not required to infer the task-belief. Hence, a permutation-invariant encoder can be used, where $q_{\phi}(z\mid c_{1:N})$ is calculated as a product of independent factors

$$q_{\phi}(z\mid c_{1:N}) \propto \prod^N_{n=1} \Psi_{\phi}(z\mid c_n)$$

To keep the method tractable, we use Gaussian factors $\Psi_{\phi}(z\mid c_n) = \mathcal{N}(f^\mu_\phi(c_n),\, f^\sigma_\phi(c_n))$, which result in a Gaussian posterior. Here $f_\phi$ is a neural network which outputs $\mu$ and $\sigma$ as mean and standard deviation of $c_n$.
